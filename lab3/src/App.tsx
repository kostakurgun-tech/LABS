...
// Замініть на свій номер у журналі (1-30)
const VARIANT = 12;
...
function App() {
  ...
  // Формуємо повний Task з даних форми (id, status, createdAt додаються тут)
  ...
  // Іммутабельне видалення — filter повертає НОВИЙ масив без задачі з цим id
  const handleDeleteTask = (id: string) => {
    setTasks((prev) => prev.filter((task) => task.id !== id));
  };

  // Іммутабельне оновлення — map повертає НОВИЙ масив,
  // для потрібної задачі створюємо НОВИЙ об'єкт через spread зі зміненим status
  ...
  // Обчислюємо перед поверненням JSX — не зберігаємо у власному useState
  const filteredTasks =
